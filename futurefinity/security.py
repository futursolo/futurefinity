...
from .utils import (
    ensure_str, ensure_bytes, deprecated_attr, Text, FutureFinityError)
...
def get_random_str(length: int) -> Text:
    """
...
class BaseContext:
    def __init__(self, security_secret: Text):
        self._security_secret = hashlib.sha256(
            ensure_bytes(security_secret)).digest()

    def lookup_origin_text(
        self, secure_text: Text,
            valid_length: Optional[int]=None) -> Optional[Text]:
        """
    ...
    def generate_secure_text(self, origin_text: Text) -> Text:
        """
...
class _HMACSecurityContext(BaseContext):
    ...
    """
    def lookup_origin_text(self, secure_text: Text,
                           valid_length: Optional[int]=None) -> Text:
        try:
